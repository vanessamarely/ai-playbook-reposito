...
| Cursor | `.cursor/rules/workspace-policy.mdc`, `.cursor/rules/style-output.mdc`, plus file-type rules, plus `.cursor/skills/` (copied from playbook, not generated) |
...
### 7. Generate Cursor Rules and Skills
...
4. As of Cursor 2.4, copy (or symlink) `<playbookRoot>/.cursor/skills/` into `<projectRoot>/.cursor/skills/` so Cursor auto-discovers the playbook's multi-step procedures natively, the same way step 6 does for Claude Code. Rules stay reserved for short always-on/glob-scoped guidance; skills carry the longer, explicitly-invoked procedures.

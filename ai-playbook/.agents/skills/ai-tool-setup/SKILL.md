...
| Tool | Files written |
|---|---|
| GitHub Copilot | `.github/copilot-instructions.md`, `.github/instructions/frontend.instructions.md`, `.github/instructions/backend.instructions.md` |
...
Verify `playbookRoot` contains `.agents/skills/` and `policies/`.
...
Map detected project types to the playbook's skills (see `<playbookRoot>/.agents/skills/*/SKILL.md` frontmatter for the canonical list — Codex does not distinguish "agent" from "skill", so all 13 procedures live under `.agents/skills/`).
...
2. Generate content using the template: `assets/cursor-rules.template.md`, splitting into one `.mdc` file per policy with correct `alwaysApply`/`globs` frontmatter.
...
2. Generate content using the template: `assets/agents-md.template.md`.
3. Write to `<projectRoot>/AGENTS.md`.
4. Optionally copy `<playbookRoot>/.agents/skills/` into `<projectRoot>/.agents/skills/` so Codex CLI discovers the playbook's skills — Codex looks under `.agents/skills` relative to the repo (and up the tree), never `.codex/skills`.
5. If a subtree needs different rules than the root (for example `src/` vs. the whole repo), add a nested `AGENTS.md` in that subtree rather than frontmatter — Codex merges the nearest file on top of the root file. See `<playbookRoot>/src/AGENTS.md` for a worked example of this pattern.
...
- Per-tool file format details: `references/tool-formats.md`
...
- Cursor rules template: `assets/cursor-rules.template.md`
- Codex AGENTS.md template: `assets/agents-md.template.md`

...
- [ ] Skill name is unique within `.agents/skills/`
...
- [ ] `description` states both what the skill does and when to use it (Codex has no separate `triggers` field — the model matches on `description` alone)
- [ ] No fields other than `name` and `description` are present in the frontmatter

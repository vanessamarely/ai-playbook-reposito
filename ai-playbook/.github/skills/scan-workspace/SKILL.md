...
Identify the project type within a target folder, determine which skills apply, and verify that AI tool instruction files (GitHub Copilot, Claude Code, Cursor, Codex CLI) are present and up to date.
...
| Cursor | `.cursor/rules/*.mdc` (an `alwaysApply: true` rule) | Optional |
| Codex CLI | `AGENTS.md` | Optional |
...
```json
{
  ...
  "warnings": [
    "WARNING: .cursor/rules/ not found. Run the ai-tool-setup skill to generate it.",
    "WARNING: AGENTS.md not found. Run the ai-tool-setup skill to generate it."
  ]
}
```
...
- Canonical Claude Code source: `.claude/agents/scan-workspace.md`
- Copilot custom agent equivalent: `.github/agents/scan-workspace.agent.md`
